...
variables {R S A B M N : Type*} [comm_ring R] [comm_ring S] [algebra R S] 
...
variables {S : Type*} [comm_semiring S] [algebra S A] [smul_comm_class R S A]
...
instance [algebra R S] [is_scalar_tower R S A] : is_scalar_tower R S (A ⊗[R] B) := ⟨λ a b c, by simp⟩
...
@[simp]
lemma include_left_apply (a : A) : (include_left : A →ₐ[S] A ⊗[R] B) a = a ⊗ₜ 1 := rfl
...
lemma product_map_right_apply (b : B) : product_map f g ((include_right : _ →ₐ[R] _) b) = g b := by simp

@[simp] lemma product_map_right : (product_map f g).comp (include_right : _ →ₐ[R] _) = g := alg_hom.ext $ by simp
