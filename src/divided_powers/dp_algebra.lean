...
lemma ideal.quotient.rel_le_ker {R : Type*} [comm_ring R] {S : Type*} [comm_ring S] (I : ideal R)
  {r : R → R → Prop} (hr : I = of_rel r) (f : R →+* S) (hf : ∀ (a b : R), r a b → f a = f b) : 
  I ≤ f.ker :=
begin
...
def ideal.quotient.lift_rel {R : Type*} [comm_ring R] {S : Type*} [comm_ring S] (I : ideal R)
{r : R → R → Prop} (hr : I = of_rel r) (f : R →+* S)
